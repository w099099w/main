...
string MD5::UcharToString(char* in)
{
	string str = in;
    return str;
}
string MD5::MD5file(string fileName)
{
    ...
    for (i = 0; i < 16; i++)
    {
        sprintf(&(output1[2 * i]), "%02x", (unsigned char)decrypt[i]);
        sprintf(&(output1[2 * i + 1]), "%02x", (unsigned char)(decrypt[i] << 4));
    }
    for (i = 0; i < 32; i++)
        output[i] = output1[i];
    fclose(fp);
